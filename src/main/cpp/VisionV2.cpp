...
*/
  #ifdef photonlibNotUsed
#include <frc/smartdashboard/SmartDashboard.h>
#include <frc/DriverStation.h>
#include <photonlib/photonlibCamera.h>
#include <photonlib/photonlibUtils.h>
#include "Const.hpp"
#include "Filter.hpp"
#include <frc/apriltag/AprilTagFieldLayout.h>

#include <photonlib/photonlibPoseEstimator.h>
...
photonlib::photonlibCamera Cam1 = photonlib::photonlibCamera("Cam1"); // the string is the name of the cam from photonlib, the name in photonlib must match this one to assign properly
...
photonlib::photonlibPoseEstimator estimator(aprilTags, photonlib::LOWEST_AMBIGUITY, std::move(Cam1), {});

photonlib::photonlibPipelineResult TagCamResult;
std::optional<photonlib::EstimatedRobotPose> CurrentEstimatedPose;
frc::Pose3d TagPose;
// vars for cone/cube cam
photonlib::photonlibCamera Cam2 = photonlib::photonlibCamera("Cam2");
photonlib::photonlibPipelineResult PieceCamResult;
photonlib::photonlibTrackedTarget PieceCamTarget;
...
void VisionInit(frc::DriverStation::Alliance LeLC_e_AllianceColor)
{
   ...
   *
   *  photonlibPoseEstimator has no c++ example code on the photonlib docs and I had to find this
   *   in the unit tests of photonlibvision
   ******************************************************************************

     frc::AprilTagFieldLayout aprilTags(aprilTagsJson);

      photonlib::photonlibPoseEstimator estimator(
 ...
 ******************************************************************************/
#ifdef OldVision
void VisionRun(photonlib::photonlibPipelineResult LsVIS_Str_TopResult,
               photonlib::photonlibPipelineResult LsVIS_Str_BottomResult,
               ...
               bool *LbVIS_b_VisionDriverModeCmndFinal)
{
  ...
  photonlib::photonlibTrackedTarget LsVIS_Str_Target;
  photonlib::photonlibPipelineResult pc_LsVIS_Str_Result[E_CamSz];
  ...
  if ((VeVIS_b_VisionDriverRequestedModeCmndLatched == false))
  {
         ...
         LeVIS_Int_Index = T_CameraLocation(int(LeVIS_Int_Index) + 1))
    {
      ...
      if (VeVIS_b_VisionTargetAquired[LeVIS_Int_Index] == true)
      {
        ...
        LeVIS_m_Range = photonlib::photonlibUtils::CalculateDistanceToTarget(
  ...
  /* Send the command out to photonlib vision: */
...
void VisionRun()
{
  ...
  if (VeVIS_b_TagHasTarget) // we only do anything if we have a target, otherwise this makes terrible data
  {
    ...
    if (CurrentEstimatedPose.has_value()) //make sure photonlib is ready before we yoink its data
    {
    ...
    OldY = V_VIS_in_TagY;
    V_VIS_in_TagX = V_VIS_m_TagX * C_MeterToIn; // photonlib outputs meters, odom wants inches
